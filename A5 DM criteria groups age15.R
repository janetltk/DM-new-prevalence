...
# Select patients diagnosed at age [age/15] years and over
#############################################################
d <- dm.criteria
...
ada <- FunBelowAge("ada") #
hku <- FunBelowAge("hku") #
hku.exMetThia <- FunBelowAge("hku.exMetThia") #

names(d) [names(d) %in% "self.reported"] <- "self"
self <- FunBelowAge("self") #
...
#############################################################
sum(is.na(d$hku) & !is.na(d$self)) # Self-reported has extra 881 cases from 1940 to 2014
...
# Merge pre-diabetes with earliest date of diagnosis including self (dm15.self)
load("diagnosis/lab_glucose.Rdata")
load("Rdata/patient.Rdata")
d <- prediabetes
d <- merge(dm15.self[, c("serial_no", "dm.type", "hku")], d, by = "serial_no", all.y = T)
...
prediabetes15 <- d
save(dm15, dm15.self, prediabetes15, file = "diagnosis/dm15.Rdata")
